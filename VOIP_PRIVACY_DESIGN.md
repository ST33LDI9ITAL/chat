...
KNOWN REMAINING (documented, not WAN-IP scope): mic-eavesdrop via forged room-key presence + unsolicited offer (PC is relay-only so no WAN leak; a hidden-call vector). Hard fix requires a per-peer NIP-04 call-acceptance handshake (unforgeable channel-membership attestation) — deferred as a separate feature. Pre-existing before this refactor.
